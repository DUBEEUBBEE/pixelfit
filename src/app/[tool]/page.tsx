...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: tool.seo.ogTitle ?? title, description: tool.seo.ogDescription ?? description, type: "website", url: publicUrl(`/${tool.slug}`), locale: brand.locale, siteName: brand.name, images: [{ url: image, width: 1200, height: 630, alt: `${tool.title} 도구 안내` }] },
  ...
  return (
    <>
      <header className="tool-hero site-shell">
        <div>
          <nav className="breadcrumbs" aria-label="현재 위치"><Link href="/">홈</Link><ChevronRight size={14} aria-hidden="true" /><span aria-current="page">{tool.title}</span></nav>
          <span className={`badge ${tool.sourceKind === "official" ? "official" : ""}`}>{tool.sourceKind === "official" && <Landmark size={12} aria-hidden="true" />}{tool.sourceKind === "official" ? "공식 출처 기반" : "서비스·일반 규격"}</span>
          <h1>{tool.title}</h1>
          <p>{tool.content.intro}</p>
        </div>
        <div className="spec-stack"><span>주요 결과</span><strong>{tool.displaySpec}</strong><span style={{ marginTop: ".65rem" }}>콘텐츠 수정</span><strong>{tool.seo.contentUpdatedAt}</strong></div>
      </header>
      ...
      <section className="section site-shell content-sections" aria-labelledby="use-cases-heading">
        ...
        <div className="content-card-grid">{tool.content.useCases.map((item) => <article className="content-card" key={item}><h3>{item}</h3><p>{tool.shortDescription}</p></article>)}</div>
      </section>
      ...
      <section className="section site-shell" aria-labelledby="source-heading">
        <div className="tool-info">
          ...
          <div>
            {tool.source ? <div className="source-box"><strong>{tool.source.authority}</strong><br />{tool.source.title}<br />출처 확인일 {tool.source.lastVerifiedAt}<br /><a href={tool.source.url} target="_blank" rel="noreferrer">공식 출처에서 다시 확인 <ExternalLink size={13} style={{ display: "inline" }} /></a></div> : <div className="source-box"><strong>서비스·일반 규격</strong><br />특정 기관이 보증한 범용 규격이 아니며 실제 제출처·플랫폼의 최신 요구가 우선합니다.</div>}
          </div>
        </div>
      </section>
      ...
      {tool.content.relatedGuideIds.length > 0 && <section className="section site-shell" aria-labelledby="related-guide-heading"><div className="section-heading"><h2 id="related-guide-heading">결과를 더 잘 쓰는 가이드</h2></div><div className="guide-link-grid">{tool.content.relatedGuideIds.map((guideId) => <Link className="guide-link-card" href={`/guide/${guideId}`} key={guideId}>{guideAnchor(guideId)} <ArrowRight size={16} aria-hidden="true" /></Link>)}</div></section>}
