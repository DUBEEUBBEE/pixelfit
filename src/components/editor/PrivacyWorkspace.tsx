  ...
  return <section className="workspace" aria-label="사진 개인정보 메타데이터 정리 도구">
    ...
    <div className="workspace-body" aria-live="polite">
      ...
      {step === 2 && file && inspection && <div className="privacy-workspace">
        <div className="success-box"><FileCheck2 size={19} /><div><strong>{file.name.replace(/[<>]/g, "")}</strong><br />실제 형식 {inspection.format.toUpperCase()} · {formatBytes(file.size)} · 사진 내용은 다시 저장하지 않습니다.</div></div>
        {!inspection.mimeMatchesSignature && <div className="error-box" role="alert">브라우저가 알려준 형식과 실제 파일 형식이 다릅니다. 신뢰할 수 있는 앱에서 다시 저장해 주세요.</div>}
        ...
        <div className="control-card"><h3><LockKeyhole size={18} />다운로드 전에 확인</h3><div className="check-list"><div className="check-item pass"><span className="check-icon"><Check size={14} /></span><div><strong>사진 내용 다시 저장하지 않음</strong><span>사진을 이루는 실제 이미지 데이터는 그대로 유지합니다.</span></div></div><div className="check-item pass"><span className="check-icon"><Check size={14} /></span><div><strong>보존 시도</strong><span>방향·DPI·색상 프로필·투명도와 알려진 출처 데이터는 선택 대상에서 제외합니다.</span></div></div><div className="check-item info"><span className="check-icon"><Info size={14} /></span><div><strong>화질 변화 예상 없음</strong><span>촬영 정보 구역만 수정하지만 모든 비식별성과 출처 정보 유효성을 보장하지 않습니다.</span></div></div></div></div>
      ...
      {step === 3 && result && inspection && <div className="result-grid">
        <div className="result-preview"><span className="icon-well" style={{ margin: "2rem auto" }}><FileCheck2 size={28} /></span><h2 style={{ margin: 0 }}>재검사 완료</h2><dl><div><dt>원본</dt><dd>{formatBytes(result.report.inputBytes)}</dd></div><div><dt>결과</dt><dd>{formatBytes(result.report.outputBytes)}</dd></div><div><dt>사진 내용 다시 저장</dt><dd>없음</dd></div><div><dt>화질 변화 예상</dt><dd>없음</dd></div></dl></div>
        <div className="result-panel"><span className="eyebrow">선택 항목 재검사</span><h2>{result.report.removedFields.length > 0 ? `${result.report.removedFields.length}개 필드를 정리했습니다.` : "제거할 알려진 필드가 없었습니다."}</h2><p>원본을 덮어쓰지 않고 새 파일로 저장합니다.</p><div className="check-list"><div className="check-item pass"><span className="check-icon"><Check size={14} /></span><div><strong>실제 사진 내용 유지</strong><span>사진을 이루는 이미지 데이터는 다시 저장하지 않았습니다.</span></div></div><div className={`check-item ${result.report.remainingSelectedCategories.length === 0 ? "pass" : "warning"}`}><span className="check-icon">{result.report.remainingSelectedCategories.length === 0 ? <Check size={14} /> : <ShieldAlert size={14} />}</span><div><strong>선택 항목 재검사</strong><span>{result.report.remainingSelectedCategories.length === 0 ? "선택한 범주의 알려진 필드가 남지 않았습니다." : `${result.report.remainingSelectedCategories.map((item) => categoryCopy[item].label).join(", ")} 일부가 안전상 남아 있습니다.`}</span></div></div><div className="check-item info"><span className="check-icon"><Info size={14} /></span><div><strong>방향·DPI·색상·투명도</strong><span>{result.report.preservation.orientationPreserved && result.report.preservation.densityPreserved && result.report.preservation.colorProfilePreserved && result.report.preservation.alphaPreserved ? "확인한 보존 대상이 그대로 유지됐습니다." : "일부 보존 항목을 확인하지 못했습니다. 다운로드 후 표시를 확인하세요."}</span></div></div></div>{result.report.provenanceMayBeInvalidated && <div className="warning-box">출처 정보는 제거하지 않았지만 파일이 바뀌어 기존 콘텐츠 자격 증명이 유효하지 않을 수 있습니다.</div>}{result.report.warnings.map((warning) => <div className="warning-box" key={warning} style={{ marginTop: ".5rem" }}>{warning}</div>)}{error && <div className="error-box" role="alert">{error}</div>}<div className="result-actions"><button className="button primary" type="button" onClick={download}><Download size={18} />정리된 파일 다운로드</button><button className="button ghost" type="button" onClick={() => setStep(2)}>선택 바꾸기</button><button className="button ghost" type="button" onClick={reset}>처음부터 다시</button></div>{tool && <NextToolActions sourceToolId={tool.id} targetIds={tool.nextToolIds} asset={result.blob} filename={createOutputFilename("photo-privacy-cleaner", inspection.format === "jpeg" ? "jpeg" : inspection.format)} />}</div>
      </div>}
    </div>
  </section>;
}
